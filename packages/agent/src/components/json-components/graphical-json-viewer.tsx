...
const TreeNode = ({ name, value, depth = 0, isLast = false, index = 0 }: TreeNodeProps) => {
  ...
  const renderValue = (val: any) => {
    ...
    switch (type) {
      ...
      case "boolean":
        return (
          <Badge variant={val ? "default" : "outline"} className={getBadgeColor(val)}>
            {val.toString()}
          </Badge>
        )
      case "null":
    ...
    >
      <div
        className={`flex items-center py-2 px-2 rounded-lg ${
          isObject ? "hover:bg-gray-100 dark:hover:bg-gray-800/50 cursor-pointer" : ""
        }`}
      ...
      >
        ...
        <span className="font-semibold mr-2 text-purple-700 dark:text-purple-400">{name}</span>

        {isObject ? (
          <Badge className="bg-gradient-to-r from-indigo-400 to-purple-500 border-0 text-white">
            {isArray ? "Array" : "Object"}
            <span className="ml-1 text-xs opacity-80">({isArray ? value.length : Object.keys(value).length})</span>
          </Badge>
        ) : (
          renderValue(value)
        )}
    ...
    <ScrollArea className="h-[500px] w-full rounded-xl border-2 border-teal-100 dark:border-teal-900/30 p-5 bg-gradient-to-br from-white to-teal-50 dark:from-gray-800 dark:to-gray-900 shadow-inner">
      <div className="space-y-1">
        {Object.entries(data).map(([key, value], index, arr) => (
          <TreeNode key={key} name={key} value={value} isLast={index === arr.length - 1} index={index} />
        ))}
      </div>
    </ScrollArea>
  )
